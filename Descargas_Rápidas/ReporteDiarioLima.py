...
def extraer_datos(ayer: datetime.date) -> dict:
    ...
    dias_rest_real = dias_reales - dias_trans
    tipo           = TIPO_DOMICILIO

    engine = get_engine()

    with engine.connect() as conn:
        # KPIs MTD + ayer en una sola consulta
        row = conn.execute(sa.text("""
            SELECT
                SUM(CASE WHEN [Tipo de domicilio] = :tipo THEN 1 ELSE 0 END)             AS ceh_mtd,
                SUM(CASE WHEN [Tipo de domicilio] = :tipo
                    AND CAST([Fecha de registro] AS DATE) = :ayer THEN 1 ELSE 0 END)     AS ceh_ayer,
                SUM(CASE WHEN CAST([Fecha de registro] AS DATE) = :ayer THEN 1 ELSE 0 END) AS total_ayer_lima,
                COUNT(*)                                                                   AS total_mtd_lima
            FROM winforce_lima
            WHERE [Estado orden] = 'Ejecutada'
            AND CAST([Fecha de registro] AS DATE) >= :primer_dia
            AND CAST([Fecha de registro] AS DATE) <= :ayer
        """), {"tipo": tipo, "ayer": ayer, "primer_dia": primer_dia}).fetchone()

        ceh_mtd         = int(row[0] or 0)
        ceh_ayer        = int(row[1] or 0)
        total_ayer_lima = int(row[2] or 0)
        total_mtd_lima  = int(row[3] or 0)

        # Top vendedores ayer C/E Hab
        rows_v = conn.execute(sa.text("""
            SELECT TOP 10 [Vendedor real], COUNT(*) AS altas
            FROM winforce_lima
            WHERE [Estado orden] = 'Ejecutada'
            AND [Tipo de domicilio] = :tipo
            AND CAST([Fecha de registro] AS DATE) = :ayer
            GROUP BY [Vendedor real]
            ORDER BY altas DESC
        """), {"tipo": tipo, "ayer": ayer}).fetchall()
        ...
        rows_z = conn.execute(sa.text("""
            SELECT [Zona_KML], COUNT(*) AS altas
            FROM winforce_lima
            WHERE [Estado orden] = 'Ejecutada'
            AND [Tipo de domicilio] = :tipo
            AND CAST([Fecha de registro] AS DATE) = :ayer
            ...
            ORDER BY altas DESC
        """), {"tipo": tipo, "ayer": ayer}).fetchall()
        por_zona = [{"Zona_KML": r[0], "altas": r[1]} for r in rows_z]

        # Altas C/E reales por semana (1-7, 8-14, 15-21, 22-fin de mes)
        semanas_def   = [(1, 7), (8, 14), (15, 21), (22, dias_reales)]
        semanas_altas = []
        for s_ini, s_fin in semanas_def:
            ...
            if f_i > ayer:
                ...
                continue
            r_s = conn.execute(sa.text("""
                SELECT COUNT(*) FROM winforce_lima
                WHERE [Estado orden] = 'Ejecutada'
                AND [Tipo de domicilio] = :tipo
                AND CAST([Fecha de registro] AS DATE) >= :f_i
                AND CAST([Fecha de registro] AS DATE) <= :f_f
            """), {"tipo": tipo, "f_i": f_i, "f_f": f_f}).fetchone()
            semanas_altas.append(int(r_s[0] or 0))

    # Métricas
    ritmo_ceh    = round(ceh_mtd / dias_trans, 1) if dias_trans > 0 else 0
    proyeccion   = round(ritmo_ceh * dias_totales)
...
def generar_pdf(d: dict) -> str:
    ...
    row_sv  = [Paragraph(sem_values[i], S["kpi_val22"]) for i in range(4)]
    row_ss  = [Paragraph(s, S["kpi_lbl"]) for s in sem_subs]

    t_sem = Table([row_sh, row_sv, row_ss],
                  colWidths=[cw4] * 4, rowHeights=[0.7*cm, 2*cm, 0.8*cm])
    ...
    # ── 4. TOP VENDEDORES ────────────────────────
    if d["top_vendedores"]:
        story.append(Paragraph("4. Top Vendedores C/E Hab. — Ayer", S["seccion"]))
        data = [["Vendedor", "Altas C/E Hab."]]
        for v in d["top_vendedores"]:
            nombre_vend = v.get("Vendedor real", v.get("vendedor", "—"))
            altas_vend  = v.get("altas", "0")
            data.append([nombre_vend, str(altas_vend)])
