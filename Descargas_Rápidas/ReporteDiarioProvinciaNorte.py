"""
ReporteDiarioProvinciaNorte.py
================================
Genera el reporte diario de Provincia NORTE de Aliv Telecom en PDF.
Consulta SQL Server directamente (tabla winforce_provincia).
Excluye Lima, Callao, Junin, Arequipa y Cusco.
Sin filtro de Tipo de domicilio.

Uso:
    python ReporteDiarioProvinciaNorte.py

El PDF se guarda como: Reporte_Diario_ProvNorte_YYYY-MM-DD.pdf
"""
...
# Departamentos excluidos: Lima/Callao + Sur (Junin, Arequipa, Cusco)
# Si en BD están con acento (JUNÍN, etc.) cambia aquí
DEPTS_EXCLUIDOS = "('LIMA', 'CALLAO', 'JUNIN', 'AREQUIPA', 'CUSCO')"

# Cuota total Provincia Norte por mes (sin Sur ni Lima)
CUOTA_POR_MES = {
    1: 700,
    2: 800,
    3: 750,
...
def extraer_datos(ayer: datetime.date) -> dict:
    mes_num      = ayer.month
    cuota        = CUOTA_POR_MES.get(mes_num, 1004)
    dias_totales = 30
    ...
    with engine.connect() as conn:
        ...
        row = conn.execute(sa.text("""
            SELECT
                COUNT(*)                                                                     AS total_mtd,
                SUM(CASE WHEN [Estado orden] = 'Ejecutada' THEN 1 ELSE 0 END)               AS altas_mtd,
                SUM(CASE WHEN CAST([Fecha de registro] AS DATE) = :ayer THEN 1 ELSE 0 END)  AS total_ayer,
                SUM(CASE WHEN [Estado orden] = 'Ejecutada'
                    AND CAST([Fecha de registro] AS DATE) = :ayer THEN 1 ELSE 0 END)        AS altas_ayer
            FROM winforce_provincia
            WHERE CAST([Fecha de registro] AS DATE) >= :primer_dia
            AND CAST([Fecha de registro] AS DATE) <= :ayer
            AND [Departamento] NOT IN ('LIMA', 'CALLAO', 'JUNIN', 'AREQUIPA', 'CUSCO')
        ...
        rows_dept = conn.execute(sa.text("""
            SELECT
                [Departamento],
                COUNT(*)                                                    AS preventas,
                SUM(CASE WHEN [Estado orden] = 'Ejecutada' THEN 1 ELSE 0 END) AS altas
            FROM winforce_provincia
            WHERE CAST([Fecha de registro] AS DATE) >= :primer_dia
            AND CAST([Fecha de registro] AS DATE) <= :ayer
            AND [Departamento] NOT IN ('LIMA', 'CALLAO', 'JUNIN', 'AREQUIPA', 'CUSCO')
    ...
    proyeccion   = round(ritmo * dias_totales)
...
def generar_pdf(d: dict) -> str:
    ayer   = d["ayer"]
    nombre = f"Reporte_Diario_ProvNorte_{ayer.strftime('%Y-%m-%d')}.pdf"
    ...
    # ── ENCABEZADO ───────────────────────────────
    story.append(Paragraph("ALIV TELECOM — WIN DISTRIBUIDORA PROVINCIA NORTE", S["titulo"]))
    story.append(Paragraph(
        f"Reporte Diario Provincia Norte · {ayer.strftime('%d/%m/%Y')} · {mes_nombre} · "
    ...
    # ── 1. SEGUIMIENTO CUOTA ─────────────────────
    story.append(Paragraph("1. Cuota del Mes — Provincia Norte", S["seccion"]))
    story.append(Paragraph(
    ...
    kpis_data = [
        ...
        ["Altas Prov. Norte MTD",       f"{d['altas_mtd']:,}",        f"Meta: {d['cuota']:,}",              d["estado"]],
    ...
    cards_l = [Paragraph(lbl, S["kpi_lbl"]) for lbl in
               ["NORTE AYER", "NORTE MTD", "PROMEDIO/DIA", "DIAS RESTANTES"]]
    ...
    story.append(Spacer(1, 4))
    story.append(Paragraph(
        f"Ayer vs promedio diario Prov. Norte: <b>{vs_str} altas</b>  ·  "
        f"Acumulado Norte MTD: <b>{d['altas_mtd']:,}</b> en {d['dias_trans']} dias.",
        S["cuerpo"]
    ))
    ...
    # ── 5. TOP VENDEDORES ────────────────────────
    if d["top_vendedores"]:
        story.append(Paragraph("5. Top Vendedores — Ayer", S["seccion"]))
        data = [["Vendedor", "Altas"]]
        for v in d["top_vendedores"]:
            data.append([v.get("Vendedor real", "—"), str(v.get("altas", "0"))])
    ...
    # ── 6. POR PLAN ──────────────────────────────
    if d["por_plan"]:
        story.append(Paragraph("6. Altas por Plan — Ayer", S["seccion"]))
        data = [["Plan", "Altas"]]
        for p in d["por_plan"]:
            data.append([p.get("Plan", "—"), str(p.get("altas", "0"))])
        t = Table(data, colWidths=[11*cm, 4*cm])
        t.setStyle(tabla_base())
        story.append(t)
        story.append(Spacer(1, 10))
...
if __name__ == "__main__":
    ...
    print(f"Generando reporte diario Provincia Norte para: {ayer.strftime('%d/%m/%Y')}")

    try:
        datos = extraer_datos(ayer)
        print(f"   Datos: {datos['altas_mtd']} altas MTD · {datos['altas_ayer']} ayer · "
              f"{datos['total_mtd']} preventas MTD · "
              f"{len(datos['por_departamento'])} departamentos Norte")
