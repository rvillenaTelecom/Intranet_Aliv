...
def main(fase: str = "bd"):
    ...
    if fase == "reporte_diario":
        ...
        ok = correr_script("ReporteDiarioLima.py", critico=True)
        if not ok:
            log.error("Pipeline detenido.")
            sys.exit(1)
    ...
    # ------------------------------------------------------------------
    # FASE REPORTE GERENTE — Vertical + Horizontal en un solo PDF
    # ------------------------------------------------------------------
    if fase == "reporte_gerente":
        separador("REPORTE GERENCIAL (ayer)")

        ok = correr_script("ReporteDiarioGerente.py", critico=True)
        if not ok:
            log.error("Pipeline detenido.")
            sys.exit(1)

        log.info("Reporte gerencial completado.")

    # ------------------------------------------------------------------
    # FASE REPORTE NOCTURNO — Igual que gerente pero con datos de hoy
    # ------------------------------------------------------------------
    if fase == "reporte_nocturno":
        separador("REPORTE NOCTURNO (hoy)")

        ok = correr_script("ReporteNocturnoGerente.py", critico=True)
        if not ok:
            log.error("Pipeline detenido.")
            sys.exit(1)
