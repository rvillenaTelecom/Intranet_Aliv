...
Subida_Referidos.py
===================
Descarga "Ventas Win Referidos" desde el Sistema de Ventas Aliv (mes actual,
filtro Canal empresa = win) y la sube a la tabla dbo.ventas_referidos en
SQL Server.

La subida es incremental por fecha: solo borra y reemplaza el rango
descargado (columna 'Fecha Ingreso'), sin tocar el historial de meses
anteriores.

Uso:
    python Subida_Referidos.py
"""
...
TABLA = "ventas_referidos"
COL_FECHA = "Fecha Ingreso"
...
def subir_referidos():
    print("=" * 50)
    print("SUBIDA REFERIDOS — DESCARGA + CARGA SQL")
    print("=" * 50)
    ...
    with sync_playwright() as p:
        ...
        try:
            if not descarga.login(page):
                ...
                return
            archivo, fecha_desde, fecha_hasta = descarga.descargar_referidos(page)
        finally:
            context.close()
            browser.close()
